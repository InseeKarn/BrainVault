(function(){
  ...
  function renderFormulaBank() {
    ...
    if (!el) return;
    el.innerHTML = formulaBank.map(f => `<button class=\"btn secondary\" data-formula=\"${f}\">${f}</button>`).join(' ');
    el.querySelectorAll('button').forEach(btn => btn.addEventListener('click', () => addStep(btn.dataset.formula)));
  }

  // Extract identifiers from a formula string
  ...
  function addStep(prefill = '') {
    ...
    // Initial render of variable fields
